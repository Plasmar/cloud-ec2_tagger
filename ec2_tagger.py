...
for i in range(length_of_original_required):
    ...
    if constant_tags[i]['Key'] in needed:
        print("Create one of these " + str(constant_tags[i].items()))
        sometags['Key'] = constant_tags[i]['Key']
        sometags['Value'] = constant_tags[i]['Value']
        temp_tags_to_add.append(sometags)
        ins.create_tags(ins.tags, Tags=temp_tags_to_add)
